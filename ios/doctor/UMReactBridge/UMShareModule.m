...
- (void)shareWithVideo:(NSString *)text icon:(NSString *)icon link:(NSString *)videoLink title:(NSString *)title platform:(NSInteger)platform completion:(UMSocialRequestCompletionHandler)completion
{
  id img = nil;
  if ([icon isEqualToString:@"thumbImage"]) {
    img = [UIImage imageNamed:@"shareimage"];
  }
  else {
    img = icon;
  }
  ...
  //创建视频内容对象
  UMShareVideoObject *shareObject = [UMShareVideoObject shareObjectWithTitle:title descr:text thumImage:img];
  //设置视频网页播放地址
  shareObject.videoUrl = videoLink;
...
RCT_EXPORT_METHOD(shareVideo:(NSString *)text icon:(NSString *)icon link:(NSString *)videoLink title:(NSString *)title platform:(NSInteger)platform completion:(RCTResponseSenderBlock)completion)
{
  ...
  [self shareWithVideo:text icon:icon link:videoLink title:title platform:plf completion:^(id result, NSError *error) {
    if (completion) {
      if (error) {
